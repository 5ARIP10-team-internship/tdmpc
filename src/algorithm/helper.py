...
def enc(cfg):
    ...
    if cfg.modality == "pixels":
        ...
        layers = [nn.Linear(cfg.obs_shape[0], cfg.enc_dim), nn.ELU(), nn.Linear(cfg.enc_dim, cfg.latent_dim)]
...
def mlp(in_dim, mlp_dim, out_dim, act_fn=nn.ELU()):
    ...
    return nn.Sequential(nn.Linear(in_dim, mlp_dim[0]), act_fn, nn.Linear(mlp_dim[0], mlp_dim[1]), act_fn, nn.Linear(mlp_dim[1], out_dim))
